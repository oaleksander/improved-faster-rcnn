...
def test(**kwargs):
    ...
    test_data_vis = Dataset(opt, mode='vis')
    test_data_test = Dataset(opt, mode='test')
    test_dataloader_vis = DataLoader(test_data_vis,
                                 batch_size=1,
                                 shuffle=False,
                                 num_workers=opt.test_num_workers)
    test_dataloader_test = DataLoader(test_data_test,
                                 batch_size=1,
                                 shuffle=False,
                                 num_workers=opt.test_num_workers)
    ...
    for i, input_data in enumerate(test_dataloader_vis):
        ...
        for bbox, label, score in zip(pred_bboxes, pred_labels, pred_scores):
            if score < 0.5:
                continue
            class_name = NEU_DET_LABEL_NAMES[label]
            # bounding box
            bbox = bbox.astype(np.int32)
            # get bbox coordinate
            ymin, xmin, ymax, xmax = bbox
            # select color
            color = VOC_COLOR_LIST[NEU_DET_LABEL_NAMES.index(class_name)]
            # draw bbox rectangle
    ...
    mAP = voc_ap(net, test_dataloader_test)
